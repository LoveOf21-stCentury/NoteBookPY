...
def show_menu(menu: text[str])-> int:
	flag = True
	for i, item in enumerate(menu):
		if i!=0:
			...
			print(item)
	input_select = input("Choose menu item: ")
	while flag:
...
def show_notes(data: text[dict[str,text[dict[str,str]]]], msg: str):
	if data:
		lists = sorted(data, key=lambda k: text(k)[0])
		print('\n' + '='*100 + '\n')
		for i in lists:
...
def add_note(input_fields: text[str])->text[dict[str,str]]:
	new_note = []
	for i in range(1):
...
def ask_change(input_fields: text[str]) -> text[str]:
    new_note = []
